...
def build_generator(dim_noise = N_DIM_RANDOM, shape_image = [28, 28]) :
	...
	model.add(keras.layers.Dense(128 * np.prod(shape_image) / 16, input_dim = dim_noise))
	model.add(keras.layers.LeakyReLU(alpha = 0.2))
	model.add(keras.layers.Reshape((int(shape_image[0] / 4), int(shape_image[1] / 4), 128)))
...
def main() :
	...
	for i_epoch in range(N_EPOCH) :
		...
		if is_bonus_discr :
			loss_gener = joint.evaluate(xx_noise_2, y_train_real)
		else :
		...
		if (i_epoch + 1) % 100 == 0 :
			...
			fname_out = "image_%05d_gan_test.csv" % (i_epoch + 1)
			write_image(x_train_gen_3, fname_out)
	# save loss
	fname_loss = "loss_gan_test.csv"
